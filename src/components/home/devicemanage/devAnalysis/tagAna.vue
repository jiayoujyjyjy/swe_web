...
export default {
  ...
  methods: {
    ...
    loaddiagram: function () {
      this.xdata = []
      for (let i = 0; i < this.tagTabData.length; i++) {
        this.xdata.push(this.tagTabData[i].tag)
      }
      console.log(this.xdata)
      this.ydata = []
      for (let j = 0; j < this.tagTabData.length; j++) {
        this.ydata.push(this.tagTabData[j].quantity)
      }
    ...
    *
    */
    // 分页获取充电桩的标签统计（表格或者图形）,默认为列表
    backChargingLabelListQue: function () {
      back.chargingLabelListQue(this.param).then(function (response) {
        console.log('分页获取充电桩的标签统计（表格或者图形）,默认为列表')
